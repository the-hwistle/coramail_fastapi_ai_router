...
def classify_email(
    ...
    return build_classification_from_llm(email, evidence, llm_data)
...
def classify_emails(
    ...
    for index, email in enumerate(selected_emails, start=1):
        ...
        classifications.append(classify_email(email, client, embed_model, llm, retrieval_limit))
...
def main() -> None:
    ...
    parser.add_argument("--db-path", type=Path, default=EMAIL_DB_PATH)
    ...
    emails = load_emails_from_sqlite(args.db_path)
    selected_emails = emails[: args.limit] if args.limit else emails

    client = QdrantClient(path=str(args.qdrant_path))
    try:
        embed_model = OllamaEmbedding(model_name=args.embedding_model)
        llm = Ollama(model=args.llm_model, request_timeout=1000.0)
